...
flowchart TD
    ...
    C --> D["new LinkInfo(...)<br/>+ UpdateSensorFrame(...)<br/>+ UpdateOdometryFrame(...)<br/>(odometry: sequential edges only)"]
...
| `lib/karto_sdk/src/Mapper.cpp` | `BOOST_CLASS_VERSION(karto::LinkInfo, 2)` — **must stay at file/global scope, not inside `namespace karto`** (see §5 gotcha). `MapperGraph::LinkScans` — the single edge-construction choke point (every edge, sequential/chain/loop-closure, funnels through here) — now also calls `UpdateSensorFrame` on every new `LinkInfo`, and `UpdateOdometryFrame` on sequential ones only (same sensor, adjacent `GetStateId()` — see §6 for why). `MapperGraph::AddEdges` — periodic-solve trigger appended at the end (increments the counter, calls `CorrectPoses()` and resets it when `enable_periodic_pose_graph_solve` is true and the threshold is hit). `MapperGraph::CorrectPoses` — feedback loop appended after the existing per-node correction loop: for each sensor in `GetSensorOffsetCorrections()`, calls `Sensor::SetOffsetPose()` and marks that sensor's scans dirty (forces cached point clouds to rebuild against the new offset). Mapper param getter/setter implementations for the two new periodic-solve params. |
...
- **Fixed per-edge odometry noise model, sequential edges only.** `odometry_edge_stddev_xy`/`_yaw` apply the same weight to every edge that has one, regardless of the distance it spans. A single fixed stddev can only be sized for one kind of span, so `LinkScans` now attaches odometry data **only to sequential edges** (same sensor, adjacent `GetStateId()`), whose span is bounded by `minimum_travel_distance`/`minimum_travel_heading`/`minimum_time_interval`. Near-chain and loop-closure edges join scans separated by an arbitrarily long driven path, over which dead reckoning has drifted far more than that stddev — including them means either stale odometry fights the loop closure and biases the extrinsic, or the stddev gets loosened to cover them and the sequential edges' information (where the calibration signal actually is) is thrown away. If long-span edges are ever wanted back, they need a distance/time-scaled stddev, not the flat one.
...
| `extrinsic_prior_stddev_xy` | `0.03` m | 1σ of the Gaussian prior anchoring `ext`'s (x,y) to nominal. Set this from the mount's actual mechanical tolerance — it is not a shrinkage knob. Loosening it to fight shrinkage (§6) was tried up to `0.1` and does not work: shrinkage is `prior_info / (prior_info + data_info)`, so loosening the prior and the odometry edges together (as happened, 0.05→0.1 alongside 0.1→0.2) divides both by the same factor and leaves the posterior mean exactly where it was, while inflating the reported covariance. A prior this wide also lets `ext` absorb systematic odometry errors (wheel radius / track width scale, EKF yaw bias) that are not mounting offset at all. Fix shrinkage on the data side instead: tighten `odometry_edge_stddev_xy`, or accumulate more edges. |
| `extrinsic_prior_stddev_yaw` | `0.03` rad (~1.7°) | Same, for yaw. `0.12` rad, tried previously, is ±14° at 2σ — a mounting mistake, not a mounting tolerance. |
| `odometry_edge_stddev_xy` | `0.03` m | 1σ of the independent odometry residual (§3.3), on **sequential edges only** (§6), which span ~`minimum_travel_distance` = 0.2 m. This is the knob that actually breaks the observability degeneracy, and the one that controls shrinkage. Size it against the EKF's real short-span error: ~1–2% of 0.2 m is a few mm, so 0.03 m is already conservative; `0.2` m, tried previously, is 1σ = 100% of the inter-node distance, which leaves ~1 unit of information per edge against ~69 for a 0.12 rad prior — i.e. tens to hundreds of edges before the estimate moves at all, which is exactly the "still shrinking toward nominal" symptom. Tightening this is safe only with a robust kernel (`ceres_loss_function`), otherwise one wheel-slip event drags `ext` with it — that (not the sigma itself) is what inflated the estimate on the aug24 bags. |
| `odometry_edge_stddev_yaw` | `0.03` rad | Same, for yaw. Note this barely affects the calibration at all: in 2D the rotation half of `AX=XB` is `R_a R_x = R_x R_b`, and planar rotations commute, so it reduces to `yaw_a = yaw_b` and carries **zero** information about `yaw_e` — the extrinsic yaw is observable only through the translation equation, i.e. through `odometry_edge_stddev_xy` and per-edge rotation. What this value does control is how hard odometry yaw pulls against the scan matcher in the map itself. |
...
| `ceres_loss_function` | `HuberLoss` | Not part of this feature (upstream param, default `None`), but load-bearing for it: it is applied to every residual `AddConstraint` adds, including the odometry one, so it is what makes a tight `odometry_edge_stddev_xy` survive an occasional slip. It also robustifies the ordinary scan-match edges — a change to the core SLAM solve, not just to calibration. |
